...
class Morphit:
    ...
    def concord(self, child, *args):
        ...
        if child in self.exceptions_list:
            return self._exception(child, args)
    ...
    def _exception(self, child, args):
...
#        def _exception(self, child, args):
#        
#        if child['lemma'] in self.exceptions[1]:
#            try:
#                if child['mode'] == 'pos':
#                    return self.preposition(child['lemma'])
#            except KeyError:
#                return self.preposition(child['lemma'])
#                
#            
#        elif child['lemma'] in self.exceptions[2]:
#            return self._indefinite_compound(child['lemma'])
#        
#        elif child['lemma'] in self.exceptions[3]:
#            return self.this()


#
#
#forme doppie (tal/tale, mangin/mangino, nessun/nessuno) andrebbero gestite meglio invece di eliminarle. Idea. Tornare anziché il primo del all_compatible form, il set dei dizionari: uniq_d = list(map(dict, frozenset(frozenset(i.items()) for i in d)))
